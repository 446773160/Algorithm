...
class Loggers(object):
    ...
    def __init__(self, filename='{date}.log'.format(date=time.strftime("%Y-%m-%d_%H%M%S", time.localtime())),
        ...
        file_handler = PathFileHandler(path=self.log_dir, filename=filename, mode='a')
        file_handler.setFormatter(format_str)
        self.logger.addHandler(stream_handler)
        self.logger.addHandler(file_handler)


log = Loggers(level='debug', log_dir=r'/home/liaosc/project/pythonProject/Park/ParkingGuide/custom/log/log/')
